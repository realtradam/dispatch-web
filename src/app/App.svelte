<script lang="ts">
  ...
  import { McpStatusView, manifest as mcpManifest, type McpStatusResult } from "../features/mcp";
  import {
    ChatLimitField,
  ...
  async function saveCompactPercent(percent: number): Promise<SaveCompactPercentResult | null> {
    const result = await store.setCompactPercent(percent);
    if (result === null) return null;
    return result.ok ? { ok: true, percent: result.percent } : { ok: false, error: result.error };
  }
  ...
  async function saveComputer(computerId: string | null): Promise<ComputerSaveResult | null> {
    ...
    if (result === null) return null;
    return result.ok
      ? { ok: true, computerId: result.computerId }
      : { ok: false, error: result.error };
  }
  ...
  const testComputer: TestComputer = async (alias: string): Promise<TestComputerResult | null> => {
  ...
  const saveSystemPromptPrompt: SaveSystemPromptAlias = (template) =>
    store.setSystemPrompt(template);
</script>
...
<main class="relative flex h-screen overflow-hidden">
  ...
  <div class="flex min-w-0 flex-1 flex-col overflow-hidden pt-[5px]">
    ...
    <Composer
      ...
      status={store.activeChat.error ? "error" : store.activeChat.generating ? "running" : "idle"}
    />
  </div>
